...
## Завершение режима водителя
`POST` `/api/v1/change/data`

Возвращает данные о пользователе: `age`, `sex`, `type`. Age в годах, например 19.
...
Надо сделать так, чтобы данные стирались после обработки, но тогда надо новую статистику делать на основе прошлой. Над этим надо полумать еще.
